...
FROM person d WHERE INNER JOIN patient_program j WHERE j.patient_id = d.person_id 
 d.person_id IN 

(--paste patient ids here--)
...
VALUES ('$arv_number','$patient_identifier','$dob','$gender','$occupation','$phone', now(), now())";
	...
	$client_medicals =  "INSERT INTO client_medicals (client_id,regimen,enrollment_date,created_at,updated_at) 
VALUES ('$new_client_id','$drug_taken','$enrollment_date', now(), now())";
